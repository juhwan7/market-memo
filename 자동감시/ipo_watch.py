...
- **현재 재료 단계:** 신규상장 완료 → 상장 후 가격·수급·실적 검증 단계
- **핵심 한줄 요약:** KRX KIND 신규상장기업현황에서 상장이 확인되어 비상장 IPO 추적 문서에서 상장 종목 분석 대상으로 전환된 초기 문서입니다.

> 이 문서는 자동 감시가 만든 **전환 초안**입니다. 자동 확인된 것은 신규상장 사실과 추출된 종목코드·상장일뿐이며, 사업·실적·밸류에이션·수급은 최신 자료를 다시 조사해야 합니다.
...
| 질문 | 답 |
|---|---|
| **지금 무슨 일이 생겼나?** | **[확정] KRX KIND에서 {company} 신규상장 확인** |
| **종목코드** | `{code}` |
| **상장일** | {listing_date_text} |
| **이 문서의 상태** | 자동감시가 만든 전환 초안 — 기업분석 미완성 |
| **★★★ 다음으로 볼 것** | 공모가 대비 주가·시가총액·보호예수·FI 지분·기관/외국인 수급 |

> **한 줄 정리:** 상장 사실은 공식 확인됐지만, 상장 후 투자판단에 필요한 기업분석은 아직 별도로 보강해야 합니다.
...
> **그래서 왜 중요?**
> `신규상장 확인`은 공식 사실이지만 `좋은 종목`이라는 판단과는 전혀 다릅니다. 자동감시는 상태만 바꾸고 투자 해석은 만들지 않습니다.
...
자동 전환 초안에서는 임의로 채우지 않습니다. 최신 공시·증권신고서·회사 IR을 확인해 보강합니다.
...
공모가·신주/구주매출·의무보유·유통가능물량을 최신 증권신고서와 거래소 자료로 확인합니다.

---

# 앞으로 볼 것

1. 상장 직후 시가총액과 공모가 대비 주가
2. 보호예수·구주매출·FI 지분 변화
3. 상장 후 첫 분기 실적
4. 외국인·기관 수급
5. 사업·실적·밸류에이션 종합 분석

> **최종 한 줄:** 이 문서는 `신규상장이라는 상태 변화`를 기록한 초안이며, 다음 업데이트에서 실제 기업분석을 채워야 합니다.

---

# 출처

- [KRX KIND 신규상장기업현황]({source_url})
- 기존 IPO 추적 문서: [`{Path(ipo_doc).name}`](../../비상장기업/{Path(ipo_doc).name})
...
- `30초 요약 → 5분 이해 → 시장 체크포인트 → 깊게 보기 → 앞으로 볼 것` 형식 자동 적용
"""
    path.parent.mkdir(parents=True, exist_ok=True)
